...
export const generateSettlementPDF = async (formData: FormData, aiResponse: AIResponse ) => {
  ...
  page.drawText(`$${aiResponse.estimateLow.toLocaleString()} - $${aiResponse.estimateHigh.toLocaleString()}`, {
    x: 50,
    y: yPosition,
    size: 18,
    font: timesRomanBold,
  ...
  aiResponse.similarCases.forEach((caseItem, index) => {
    page.drawText(`${index + 1}. ${caseItem.title} - $${caseItem.outcome.toLocaleString()}`, {
      x: 50,
      y: yPosition,
      size: 12,
      font: timesRomanBold,
