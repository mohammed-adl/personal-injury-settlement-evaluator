...
export const injuryFormDataSchema = injurySubmissionSchema.transform(({ data }) => {
  ...
  const treatmentLevelField = data.fields.find(f => f.label.trim() === "Treatment level");

  let treatmentLevel = "Unknown";
  if (Array.isArray(fields["Treatment level"]) && fields["Treatment level"].length > 0) {
    const selectedId = fields["Treatment level"][0];
   
    
    const option = treatmentLevelField?.options?.find(opt => opt.id === selectedId);
